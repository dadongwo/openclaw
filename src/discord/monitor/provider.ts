...
export async function monitorDiscordProvider(opts: MonitorDiscordOpts = {}) {
  ...
  const applicationSummary = await fetchDiscordApplicationSummary(token, 4000, discordRestFetch);
  const applicationId =
    applicationSummary?.id ?? (await fetchDiscordApplicationId(token, 4000, discordRestFetch));
  if (!applicationId) {
    throw new Error("Failed to resolve Discord application id");
  }
  const messageContentStatus = applicationSummary?.intents?.messageContent;
  if (
    messageContentStatus === "disabled" &&
    discordCfg.intents?.messageContent === undefined
  ) {
    runtime.log?.(
      warn("discord: Message Content Intent disabled in Portal; running in mention-safe mode."),
    );
    discordCfg.intents = { ...(discordCfg.intents ?? {}), messageContent: false };
  }
